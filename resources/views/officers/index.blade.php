                        ...
                        <tr x-show="matchRow('{{ addslashes($officer->officer_code) }}', '{{ addslashes($officer->name) }}', '{{ addslashes($officer->user->email ?? '') }}', '{{ $officer->status }}')" 
                            ...
                            <td class="px-4 py-3.5 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center gap-2 justify-end">
                                    ...
                                    <form method="POST" action="{{ route('officers.force-delete', $officer) }}"
                                          onsubmit="return confirm('HAPUS PERMANEN petugas {{ addslashes($officer->name) }} beserta akun login-nya? Data yang sudah dihapus tidak bisa dikembalikan.');" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
