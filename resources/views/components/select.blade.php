...
@php
    $id = $id ?? $attributes->get('name', uniqid('select_'));
...
<div class="w-full">
    ...
    <select
        id="{{ $id }}"
        {{ $attributes->merge([
            'class' => 'w-full px-3.5 py-2 text-sm rounded-lg border transition-colors bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-offset-1 ' .
                ($hasError 
