                        ...
                        <tr x-show="matchRow('{{ addslashes($location->code) }}', '{{ addslashes($location->name) }}', '{{ $location->type }}')" 
                            ...
                            <td class="px-4 py-3.5 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center gap-2 justify-end">
                                    ...
                                    <form method="POST" action="{{ route('locations.force-delete', $location) }}"
                                          onsubmit="return confirm('HAPUS PERMANEN lokasi {{ addslashes($location->name) }}? Data yang sudah dihapus tidak bisa dikembalikan.');" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
